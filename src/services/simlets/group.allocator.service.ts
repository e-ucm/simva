 ...
 * ```
 */
export async function getAllocatorFromSimlet(simletId: number, group_id: number, current_user_id: number): Promise<Allocator> {
  throw new NotImplementedError("Allocation of participants to sessions and activities is now managed at the group level. Use Group.group_allocator_type instead.");
  //let simlet = await Simlet.getFromDbData(simletId, current_user_id);
  //return await simlet.getAllocator();
}

export async function updateSimletAllocator(simletId: number, group_id: number, current_user_id: number, body: any): Promise<Allocator> {
  throw new NotImplementedError("Allocation of participants to sessions and activities is now managed at the group level. Use Group.group_allocator_type instead.");
  //let simlet = await Simlet.getFromDbData(simletId, current_user_id);
...
export async function allocateToSessionSimlet(simletId: number,  group_id: number, sessionId: number, current_user_id: number, objectId?: number): Promise<Allocator> {
  throw new NotImplementedError("Allocation of participants to sessions and activities is now managed at the group level. Use Group.group_allocator_type instead.");
    //let simlet = await Simlet.getFromDbData(simletId, current_user_id);
  //return await simlet.allocateToSession(sessionId, objectId);
}