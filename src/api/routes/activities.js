...
/**
 * Obtains the requested activity export
 * 
 */
router.get('/:id/export', Authenticator.auth, async (req, res, next) => {
  const options = {
    id: req.params['id'],
    user: req.user,
    complete: (req.query.complete.toLowerCase() === "true")
  };

  try {
    const result = await activities.exportActivity(options);
    res.status(result.status || 200).send(result.data);
  } catch (err) {
    next(err);
  }
...
/**
 * Get survey languages of an existing Limesurvey activity
 */
router.get('/:id/surveylanguages', Authenticator.auth, async (req, res, next) => {
  const options = {
    id: req.params['id'],
    user: req.user
  };
  ...
  try {
    const result = await activities.getSurveyLanguages(options);
    res.status(result.status || 200).send(result.data);
  } catch (err) {
    next(err);
  }
 ...
 * 
 */
router.get('/:id/usersurveylist', Authenticator.auth, async (req, res, next) => {
  const options = {
    user: req.user
  };
  try {
    const result = await activities.getUserSurveys(options);
    res.status(result.status || 200).send(result.data);
  } catch (err) {
    next(err);
  }
