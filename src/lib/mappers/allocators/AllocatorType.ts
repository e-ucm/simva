...
export class AllocatorType {
	...
	static async getAll(type?: string[]): Promise<AllocatorType[]> {
        if(!type) {
            type = [Allocator.getType(), GroupAllocator.getType(), SessionAllocator.getType(), RandomAllocator.getType()];
        }
        const types : any[] = [];
        if(type.includes(Allocator.getType())) types.push(Allocator);
        if(type.includes(GroupAllocator.getType())) types.push(GroupAllocator);
        if(type.includes(SessionAllocator.getType())) types.push(SessionAllocator);
        if(type.includes(RandomAllocator.getType())) types.push(RandomAllocator);
