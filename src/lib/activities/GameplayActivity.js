...
class GameplayActivity extends Activity {
	...
	async save(){
		...
		if(!this.id){
			if(this.extra_data.config && this.extra_data.config.realtime){
	...
	async setResult(participant, result){
		...
		try{
			if(Array.isArray(result)){
				...
				if(this.extra_data.config.trace_storage || this.extra_data.config && this.extra_data.config.realtime){
					if(this.extra_data.config.trace_storage){
						var traces= [];
						for(let traceId = 0; traceId < result.length; traceId++) {
							var trace = result[traceId];
	...
	async getResults(participants, type){
		...
		for (var i = participants.length - 1; i >= 0; i--) {
			...
			if( (this.extra_data.config && this.extra_data.config.realtime && analyticsresults[participants[i]] !== null) 
